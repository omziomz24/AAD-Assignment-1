...
lm.fit <- lm(medianHouseValue~poly(longitude,3),data = data) 
summary(lm.fit)
plot(lm.fit)

# latitude, again non-linear poly?
lm.fit <- lm(medianHouseValue~poly(latitude,3),data = data) 
summary(lm.fit)
plot(lm.fit)
...
# had to comment out for time being cause couldn't run this section - omar
...
lm.fit <- lm(medianHouseValue~.-id-bedroomsPerRoom-distToCenter-incomePerRoom,data = data) # ID not a relevant predictor
summary(lm.fit)
...
# k-fold CV
# best so far
#medianHouseValue~.-id-oceanProximity-aveBedrooms-aveRooms-population+bedroomsPerRoom+longitude:latitude+log(population)+I(medianIncome^2)+I(medianIncome^3)+I(housingMedianAge^2)+medianIncome:log(population)+bedroomsPerRoom:medianIncome+medianIncome:housingMedianAge+distToCenter+distToCenter:medianIncome+housingMedianAge: bedroomsPerRoom+incomePerRoom:bedroomsPerRoom+housingMedianAge:distToCenter

#other I can add: aveRooms:medianIncome+housingMedianAge:population+latitude:incomePerRoom+longitude:housingMedianAge

lm.fit <- lm(medianHouseValue~.-id-oceanProximity-aveBedrooms-aveRooms-population+bedroomsPerRoom+longitude:latitude+log(population)+I(medianIncome^2)+I(medianIncome^3)+I(housingMedianAge^2)+medianIncome:log(population)+bedroomsPerRoom:medianIncome+medianIncome:housingMedianAge+distToCenter+distToCenter:medianIncome+housingMedianAge: bedroomsPerRoom+incomePerRoom:bedroomsPerRoom+housingMedianAge:distToCenter+latitude:incomePerRoom+longitude:housingMedianAge,data = data)
summary(lm.fit)
