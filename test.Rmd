...
For longitude and latitude, it is apparent that a polynomial model with degree 2 and 3 is better than the linear model. 
```{r}
...
#### Housing Median Age: want to justify why degree 2 polynomial is better 
```{r}
...
lm.housingMedianAge.poly2 <- lm(medianHouseValue 
                                ...
                                data = data)
lm.housingMedianAge.poly3 <- lm(medianHouseValue 
                                ~ poly(housingMedianAge, 3, raw = T), 
                                data = data)
par(mfrow = c(2,2)); plot(lm.housingMedianAge, cex = 0.1)
par(mfrow = c(2,2)); plot(lm.housingMedianAge.poly2, cex = 0.1)
anova(lm.housingMedianAge, lm.housingMedianAge.poly2, lm.housingMedianAge.poly3 ) 
```
...
Further, the predictor "population" suffers from high leverage point and heteroskedasticity. To address this issue, a log transform is applied to the original data. From the pre-log-transform residual plot, the residuals are skewed/clustered at one end, whereas the post-log plot show better random scattering. This indicates that a log transform improves linearity between the response and the predictor, which motivates us to implement this improvement in our final model. 
```{r}
