...
There are 18640 data points in the original data set. There are 10 features being observed and recorded, inclduing the response variable, Median House Value ("medianHouseValue"). 
```{r}
...
```
When inspecting the orginal data, we observe that there are some missing values (NA's) in aveBedrooms. 
```{r }
# NA's
