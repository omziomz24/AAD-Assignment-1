...
Some relationships are clearly non-linear so have to consider transformations this is an example of an approach to take for pop
...
Please do this for: I(medianIncome\^2) I(medianIncome\^3) I(housingMedianAge\^2) I(latitude\^2) I(longitude\^2) I(latitude\^3) I(longitude\^3) log(population)
...
```{r}
# BEDROOMS PER ROOM NOT FOUND ERROR!!!!!!!!!!!!!!1
lm.fit <- lm(medianHouseValue~bedroomsPerRoom,data = data) 
summary(lm.fit)
plot(lm.fit)
...
From the Residuals vs Fitted plot it is clear that the fit has improved significantly since the original model and the Scale Location plot displays less heteroscedasticity than the previous model. However one cause for concern is the significant leverage (for one row leverage is 1). However from further investigation it is clear that the the highly leveraged points relate to the four row where oceanProximity is ISLAND. If they are removed ISLAND can't be used a predictor and hence neither than oceanProximity. This high leverage for a minority of houses is where oceanProximity is ISLAND is a trade-off of the final model.
...
Please use reg.full.summary for three most important Use p-values for the other important variables - remember to check covariance...
...
best_mse # Chosen test MSE for competition
```
