...
data <- na.omit(data) # can remove column aveBedrooms???
...
lm.fit <- lm(medianHouseValue~.-id-aveBedrooms,data = data) # ID not a relevant predictor
summary(lm.fit)
...
library(car)
# k-fold CV
lm.fit <- lm(medianHouseValue~.-id-oceanProximity-aveBedrooms+longitude:latitude+log(population),data = data)
summary(lm.fit)
...
for(j in 1:k){
  best.fit=regsubsets(medianHouseValue~.-id-oceanProximity-aveBedrooms+longitude:latitude+log(population),data = data[folds!=j,],nvmax=nvmax)
  for(i in 1:nvmax){
...
reg.best=regsubsets(medianHouseValue~.-id-oceanProximity-aveBedrooms+longitude:latitude+log(population),data = data, nvmax=nvmax)
...
``` {r}
test_data <- data <- read.csv("Assignt1_data.csv")
