...
library(car)
# k-fold CV
#medianHouseValue~.-id-oceanProximity-medianIncome+log(medianIncome)+longitude:latitude+log(population)
lm.fit <- lm(medianHouseValue~.-id-oceanProximity-aveRooms-aveBedrooms+longitude:latitude+log(population),data = data)
summary(lm.fit)
...
for(j in 1:k){
  best.fit=regsubsets(medianHouseValue~.-id-oceanProximity-aveRooms-aveBedrooms+longitude:latitude+log(population),data = data[folds!=j,],nvmax=nvmax)
  for(i in 1:nvmax){
...
reg.best=regsubsets(medianHouseValue~.-id-oceanProximity-aveRooms-aveBedrooms+longitude:latitude+log(population),data = data, nvmax=nvmax)
...
coef(ridge.model) #not very good!
coef(model.best)
