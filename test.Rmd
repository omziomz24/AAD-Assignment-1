...
When inspecting the orginal data, we observe that there are 190 rows that have missing values (NA's) in the varaible "aveBedrooms". Since using data points with missing values can lead ot baised results, and since 190 out of 18,640 data points is an imaterial proportion, we decided to remove these rows that contains NA's from the original data frame.
```{r }
...
By performing single linear regressions, we find that individual pair of relationships are all statistically significant. However, some assumptions necessary for OLS are clearly violated (e.g. heteroskedasiticty), and some relationship are clearly non-linear. We therefore consider polynomial and log transformations. 

For longitude and latitude, it is apparent that a polynomial model with degree 2 and 3 is much better than the linear model. 
```{r}
lm.latitude = lm(medianHouseValue ~ latitude, data = data)
lm.latitude.poly2 = lm(medianHouseValue ~ I(latitude^2), data = data)
lm.latitude.poly3 = lm(medianHouseValue ~ I(latitude^3), data = data) 
par(mfrow = c(2,2))
...
We have also explored polynomial regression on medianIncome: 
```{r}
lm.medianIncome <- lm(medianHouseValue ~ medianIncome, data = data)
summary(lm.medianIncome)
par(mfrow = c(2,2)); plot(lm.medianIncome, cex = 0.1)

lm.medianIncome.poly2 <- lm(medianHouseValue ~ I(medianIncome^2), data = data)
summary(lm.medianIncome.poly2)
par(mfrow = c(2,2)); plot(lm.medianIncome.poly2, cex = 0.1)

lm.medianIncome.poly3 <- lm(medianHouseValue ~ I(medianIncome^3), data = data)
summary(lm.medianIncome.poly3)
par(mfrow = c(2,2)); plot(lm.medianIncome.poly3, cex = 0.1)
```

When we tried quadratic model for housingMedianAge, it does not seem to improve the model any further. The residual 
```{r}
...
lm.housingMedianAge.poly2 <- lm(medianHouseValue ~ I(housingMedianAge^2), data = data)
summary(lm.housingMedianAge.poly2)
par(mfrow = c(2,2)); plot(lm.housingMedianAge.poly2, cex = 0.1)
```

Further, the predictor "population" suffers from high leverage point and heteroskedasticity. To address this issue, a log transform is applied to the original data. From the pre-log-transform residual plot, the residuals are skewed/clustered at one end, whereas the post-log plot show better symmetry. This indicates that a log transform improves linearity between the response and the regressor, which motivates us to implement this improvement in the multiple regression later on. 
```{r}
